...
class Util:
    ...
    def save_test_predictions(self, test_targets, test_outputs):
        predictions_dir = self.__create_dir('predictions')
        targets_filename = os.path.join(predictions_dir, self.base_filename + '-targets.pt')
        outputs_filename = os.path.join(predictions_dir, self.base_filename + '-outputs.pt')
        pt_targets = torch.cat(test_targets, dim=0)
        pt_outputs = torch.cat(test_outputs, dim=0)
        torch.save(pt_targets, targets_filename)
        torch.save(pt_outputs, outputs_filename)
...
class ImgUtil():
    def __init__(self, data, titles, _min, _max):
        self.data = data
        self.titles = titles
        self.fig, self.axs = plt.subplots(1, len(data), figsize=(20,10))
        self.im = []
        self.min = _min
        self.max = _max
        
    def get_animation_gif(self, frames, cmap="Blues"):
        for i, d in enumerate(self.data):
            self.axs[i].title.set_text(self.titles[i])
            self.axs[i].title.set_fontsize(18)
            self.im.append(self.axs[i].matshow(d[0,:,:], vmin=self.min, vmax=self.max, cmap=cmap))
            
        return animation.FuncAnimation(self.fig, self.__animate, init_func=self.__init, frames=frames, repeat=True)
    
    def __init(self):
        for i, d in enumerate(self.data):
            self.im[i].set_data(d[0,:,:])

    def __animate(self, step):
        for i, d in enumerate(self.data):
            self.im[i].set_data(d[step,:,:])